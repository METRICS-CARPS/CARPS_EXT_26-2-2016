...
```{r}
error_latency_rows <- spread_data %>%
  filter(measurement_type == "_ERROR_latency")
colnames(error_latency_rows)[121] = "ERROR_latency"
dim(error_latency_rows)

correct_latency_rows <- spread_data %>%
  filter(measurement_type == "_CORRECT_latency")
colnames(correct_latency_rows)[121] = "CORRECT_latency"
dim(correct_latency_rows)


latency_rows <- spread_data %>%
  filter(measurement_type == "_latency")
colnames(latency_rows)[121] = "latency"
dim(latency_rows)

stimulus_rows <- spread_data %>%
  filter(measurement_type == "_stimulus")
colnames(stimulus_rows)[121] = "stimulus"
dim(stimulus_rows)

cannonical_data <- error_latency_rows
cannonical_data$CORRECT_latency <- correct_latency_rows$CORRECT_latency
cannonical_data$latency <- latency_rows$latency
cannonical_data$stimulus <- stimulus_rows$stimulus
dim(cannonical_data)
```
...
filtered_data_manual <- data[!is.element(data$id, bad_ids),]
...
filtered_data_manual <- data[data$IAT2766_status=="completed",]

filtered_data <- filter(data, `Filter_$` == 1)
...
```{r}
mean(gathered_data$IATscore, na.rm=TRUE)
```
